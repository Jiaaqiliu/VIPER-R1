...
class Qwen2_5_VLGRPOVLLMTrainerModified(Trainer):
    def __init__(
        self,
    ...
    def load_images_safe(self, image_paths):
        pil_images = []
    ...
    def _prepare_inputs(
        ...
        a, b = self.load_image(inputs[0]["image_phase"]), self.load_image(inputs[0]["image_xt"])
        images = [a,b]
        # images = [[self.load_image(x["image_phase"]), self.load_image(x["image_xt"])] for x in inputs]
        # images = [x["image_phase"] for x in inputs] + [x["image_xt"] for x in inputs]
        # images = [x["image"] for x in inputs]
        # images = [x["image_phase"] for x in inputs] + [x["image_xt"] for x in inputs]
        # images = [[x["image_phase"], x["image_xt"]] for x in inputs]
        # images = [x["image_phase"] for x in inputs]
